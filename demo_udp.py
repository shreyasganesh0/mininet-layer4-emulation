...
def main():
    ...
    print_banner("LIVE DEMO: UDP PERFORMANCE TEST")
    ...
    try:
        ...
        print_section("STEP 5: Running UDP Performance Test")
        
        print("Protocol: UDP (User Datagram Protocol)")
        print("Congestion Control: NONE (connectionless)")
        print("Test Duration: 30 seconds")
        print("Target Bandwidth: 20 Mbps")
        print("Command: iperf3 -c 10.0.0.13 -u -b 20M -t 30\n")
        
        print("About UDP:")
        print("  • Connectionless protocol - no handshake")
        print("  • NO congestion control - sends at specified rate")
        print("  • Lower latency but unreliable delivery")
        print("  • Will reveal network capacity limits through packet loss\n")
        
        print("Expected behavior:")
        print("  • In DEBUG mode: Low loss, high throughput")
        print("  • In PROJECT mode: HIGH packet loss (bottleneck is 10 Mbps!)")
        print("  • UDP doesn't back off - continues transmitting at 20 Mbps")
        print("  • Demonstrates the need for congestion control\n")
        ...
        output = client.cmd('iperf3 -c 10.0.0.13 -u -b 20M -t 30')
        print(output)
        ...
        output_json = client.cmd('iperf3 -c 10.0.0.13 -u -b 20M -t 5 -J')
        
        if not os.path.exists('experiment_results'):
            os.makedirs('experiment_results')
        
        mode = 'bottleneck' if use_bottleneck else 'debug'
        filename = f'experiment_results/{controller_name}_{mode}_udp_iperf.json'
        ...
        # Save ping results
        ping_filename = f'experiment_results/{controller_name}_{mode}_udp_ping.txt'
        with open(ping_filename, 'w') as f:
            f.write(ping_output)
        
        print(f"\n✓ Ping results saved to: {ping_filename}\n")
        
        print_banner("UDP DEMO COMPLETE")
        
        print("Key Observations:")
        print("  • UDP achieves highest throughput but with massive packet loss")
        print("  • In bottleneck mode: ~50% packet loss (sending 20 Mbps into 10 Mbps link)")
        print("  • No congestion control = no adaptation to network conditions")
        print("  • Demonstrates fundamental tradeoff: speed vs. reliability\n")
        
        print("Why UDP has high packet loss:")
        print("  • Bottleneck link: 10 Mbps capacity")
        print("  • UDP transmission: 20 Mbps (2x the capacity)")
        print("  • No backoff mechanism when packets are dropped")
        print("  • Result: ~50% of packets must be dropped\n")
        
        print("Comparison with TCP:")
        print("  • TCP protocols detect congestion and slow down")
        print("  • UDP continues at full speed regardless of loss")
        print("  • UDP suitable only when loss is acceptable (video streaming)")
        print("  • TCP necessary when reliability matters\n")
        
        print("Next steps:")
        print("  1. Run analysis to compare all protocols: python3 analyze_results.py")
        print("  2. Review packet loss differences between UDP and TCP")
        print("  3. Understand why congestion control matters\n")
