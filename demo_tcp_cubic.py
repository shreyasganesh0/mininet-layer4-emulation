#!/usr/bin/env python3
"""
LIVE DEMO: TCP CUBIC Performance Test
...
def main():
    ...
    print_banner("LIVE DEMO: TCP CUBIC PERFORMANCE TEST")
    ...
    try:
        ...
        print_section("STEP 5: Running TCP CUBIC Performance Test")
        
        print("Protocol: TCP CUBIC")
        print("Congestion Control: CUBIC (cubic window growth)")
        print("Test Duration: 30 seconds")
        print("Command: iperf3 -c 10.0.0.13 -C cubic -t 30\n")
        
        print("About TCP CUBIC:")
        print("  • Default congestion control in Linux since kernel 2.6.19")
        print("  • Uses cubic function for window growth (not linear)")
        print("  • Better performance in high-bandwidth, high-latency networks")
        print("  • More aggressive recovery after packet loss\n")
        ...
        output = client.cmd('iperf3 -c 10.0.0.13 -C cubic -t 30')
        print(output)
        ...
        output_json = client.cmd('iperf3 -c 10.0.0.13 -C cubic -t 5 -J')
        
        if not os.path.exists('experiment_results'):
            os.makedirs('experiment_results')
        
        mode = 'bottleneck' if use_bottleneck else 'debug'
        filename = f'experiment_results/{controller_name}_{mode}_cubic_iperf.json'
        ...
        # Save ping results
        ping_filename = f'experiment_results/{controller_name}_{mode}_cubic_ping.txt'
        with open(ping_filename, 'w') as f:
            f.write(ping_output)
        
        print(f"\n✓ Ping results saved to: {ping_filename}\n")
        
        print_banner("TCP CUBIC DEMO COMPLETE")
        
        print("Key Observations:")
        print("  • TCP CUBIC typically achieves higher throughput than Reno")
        print("  • Cubic window growth adapts better to network conditions")
        print("  • More efficient recovery from congestion events")
        print("  • Modern standard for TCP congestion control\n")
        
        print("Comparison with TCP Reno:")
        print("  • CUBIC should show better throughput in congested networks")
        print("  • Window growth is more aggressive near previous maximum")
        print("  • Better utilization of available bandwidth\n")
        
        print("Next steps:")
        print("  1. Run demo for UDP: sudo python3 demo_udp.py")
        print("  2. Analyze all results: python3 analyze_results.py")
        print("  3. Compare CUBIC vs Reno performance\n")
